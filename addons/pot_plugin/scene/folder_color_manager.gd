...
##icon:Color
##bg:Color
func get_dir_or_file_color(dir:StringName) -> Dictionary[StringName, Color]:
	var color_dic:Dictionary[StringName, Color]
	
	##---------------------------------------------------------
	##先祖の一番近い色付きフォルダーの位置と色の名前を取得 ない場合は空
	var ps:Dictionary[StringName, StringName] = {}
	
	if ProjectSettings.has_setting("file_customization/folder_colors"):
		ps.assign(ProjectSettings.get_setting("file_customization/folder_colors") )
	
	
	var max_length:int = 0
	var final_folder:StringName
	for folder:StringName in ps.keys():
...
const FOLDER_COLOR_LIST:Dictionary[StringName, Color] = {
...
func get_color_from_folder_color_name(color_name:StringName) -> Color:
